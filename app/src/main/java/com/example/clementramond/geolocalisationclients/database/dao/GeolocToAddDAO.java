...
public class GeolocToAddDAO extends GeolocClientsDBDAO {

    private static final String WHERE_ID_EQUALS =
        DBHelper.GEOLOC_TO_ADD_COLUMNS[DBHelper.GEOLOC_TO_ADD_DATETIME] + " = ?"
            + " and " + DBHelper.GEOLOC_TO_ADD_COLUMNS[DBHelper.GEOLOC_TO_ADD_UTILISATEUR] + " = ?";

    public GeolocToAddDAO(Context context) {
        super(context);
    }

    public long save(Geolocalisation geoloc) {
        return database.insert(DBHelper.TABLE_GEOLOC_TO_ADD, null,
            toContentValues(geoloc));
    }

    public long update(Geolocalisation geoloc) {
        long result = database.update(DBHelper.TABLE_GEOLOC_TO_ADD,
    ...
    public int delete(Geolocalisation geoloc) {
        return database.delete(DBHelper.TABLE_GEOLOC_TO_ADD, WHERE_ID_EQUALS,
            new String[] {
                String.valueOf(geoloc.getDateTime().format(Geolocalisation.MYSQL_DTF)),
                String.valueOf(geoloc.getUtilisateur().getPseudo())
            });
    ...
    private Cursor getCursorAll() {
        String requete = DBHelper.getSelectFromGeolocToAdd()
            + " order by " + DBHelper.GEOLOC_TO_ADD_COLUMNS[DBHelper.GEOLOC_TO_ADD_DATETIME];
    ...
    private ArrayList<Geolocalisation> cursorToListe(Cursor c) {
        ...
        if (c.getCount() > 0) {
            while (c.moveToNext()) {
                ...
                geoloc.setDateTime(LocalDateTime.parse(c.getString(DBHelper.GEOLOC_TO_ADD_DATETIME),
                    Geolocalisation.MYSQL_DTF));

                utilisateur = new Utilisateur();
                utilisateur.setPseudo(c.getString(DBHelper.GEOLOC_TO_ADD_UTILISATEUR));
                ...
                geoloc.setLatitude(c.getDouble(DBHelper.GEOLOC_TO_ADD_LATITUDE));
                geoloc.setLongitude(c.getDouble(DBHelper.GEOLOC_TO_ADD_LONGITUDE));
    ...
    public static ContentValues toContentValues(Geolocalisation geoloc) {
        ...
        values.put(DBHelper.GEOLOC_TO_ADD_COLUMNS[DBHelper.GEOLOC_TO_ADD_DATETIME], geoloc.getDateTime().format(Geolocalisation.MYSQL_DTF));
        values.put(DBHelper.GEOLOC_TO_ADD_COLUMNS[DBHelper.GEOLOC_TO_ADD_UTILISATEUR], geoloc.getUtilisateur().getPseudo());
        values.put(DBHelper.GEOLOC_TO_ADD_COLUMNS[DBHelper.GEOLOC_TO_ADD_LATITUDE], geoloc.getLatitude());
        values.put(DBHelper.GEOLOC_TO_ADD_COLUMNS[DBHelper.GEOLOC_TO_ADD_LONGITUDE], geoloc.getLongitude());
